...
- [x] **Extended Health Metrics**: `HealthServer` exposes `uptime` (ms since start) and `queueLength` (pending Redis Stream messages) via `/health`.
- [x] **3Proxy Rate Limiting**: `maxconn` parameter added to each proxy block in the generated config. Default is `64` (via `CONFIG.MAX_CONN_PER_PROXY`), overridable with env var `PROXY_MAX_CONN`.
